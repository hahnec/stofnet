...
class sinc_conv(nn.Module):
    ...
    def __init__(self, N_filt, Filt_dim, fs):
        ...
        self.filt_band = nn.Parameter(
            torch.from_numpy((b2-b1)/self.freq_scale))
    ...
    def forward(self, x):
        ...
        N = self.Filt_dim
        t_right = Variable(torch.linspace(
            1, (N-1)/2, steps=int((N-1)/2))/self.fs).cuda()
        ...
        for i in range(self.N_filt):
            ...
            low_pass1 = 2 * \
                filt_beg_freq[i].float()*sinc(filt_beg_freq[i].float()
                                              * self.freq_scale, t_right)
            low_pass2 = 2 * \
                filt_end_freq[i].float()*sinc(filt_end_freq[i].float()
                                              * self.freq_scale, t_right)
       ...
       for i in range(self.N_cnn_lay):
         ...
         if i == 0:
          if self.use_sinc:
            ...
            self.conv.append(
                nn.Conv1d(1, self.cnn_N_filt[i], self.cnn_len_filt[i]))
